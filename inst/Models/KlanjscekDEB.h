/*
  KlanjscekDEB.h -  Header file specifying the elementary life-history functions of
  ...
  if (lifestage[0] == 1)                                                            // Only for adults
    {
      ...
      fecundity[0][0] = max(Er - (1 - KAPPA)*EM*M*G*VP, 0);
      fecundity[0][0] /= EM*(KAPPA*G + FOOD)*VB/KAPPA_R;
    }
  else
    fecundity[0][0] = 0;
