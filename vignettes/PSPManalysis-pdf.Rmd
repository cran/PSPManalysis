...
output:
  pdf_document:
    ...
    pandoc_args: [--template, ./PSPManalysis-template.tex, -f, markdown-markdown_in_html_blocks, --listings, -H, ./listings-setup.tex]
    toc: yes
    toc_depth: 3
...
The last possible element of the option vector that modifies the behavior of the computational module is the \texttt{"report"} option, which can be passed to the computational module by using for example \texttt{c("report", "10")} as fourth argument to the \texttt{PSPMdemo} function. This option determines how much output the computational module reports to the console. The default value of \texttt{"report"} equals 1, which implies that the software writes the values of every new solution point that it has computed to the \texttt{R} console. A value of 2 for the \texttt{"report"} option means that every other computed solution point is written to the \texttt{R} console, whereas the specification of \texttt{c("report", "10")} as fourth argument to the \texttt{PSPMdemo} function implies that every 10th solution point that is computed is written to the \texttt{R} console.

If necessary the options \texttt{"test"}, \texttt{"isort"} and \texttt{"report"} can be combined in arbitrary order, for example, as
...
* Option \texttt{silent}: When \texttt{silent=TRUE} is passed as argument, it instructs the \texttt{PSPMdemo} function to suppress all messages from the compilation of the model specific file into a dynamic library module. This option is useful to prevent cluttering the console with superfluous messages once a model specific file has been tested sufficiently and functions without problems.
...
\begin{align*}\displaystyle
	...
	&\dfrac{dP}{dt}\;=\;\left(\epsilon\dfrac{aB}{1+aT_hB}\:-\:\delta\right)\,P\\[2ex]
...
$$\mu(\ell,P)=
	\begin{cases}
		\mu_b\,+\,\dfrac{aP}{1+aT_hB}\qquad&\textrm{if $\ell<\ell_v$}\\
...
The threshold value that has to be stored and returned to the program in \texttt{limit[p]} will depend on the individual state variables, possibly on the individual's state-at-birth and will be different for individuals in different life stages. For this reason, the routine \texttt{IntervalLimit()} has as arguments \texttt{lifestage[]}, specifying the life stage that the individual is currently in, \texttt{istate[][]}, the individual state, \texttt{birthstate[][]} and \texttt{BirthStateNr}, the value and index of the individual's state-at-birth, respectively. The threshold value marking the end of a particular stage may, however, in addition depend on the value of the environment variables (\texttt{E[]}).
...
This routine is called whenever a transition between two consecutive life stages is reached during the integration over the individual life history. It should be noted that the value of the variable \texttt{lifestage[}$p$\texttt{]} indicates the life stage that is entered, that is, following the current stage boundary. This routine will hence never be called with a value of one of the elements \texttt{lifestage[}$p$\texttt{]} equal to 0. The discrete changes in the individual state variables have to be implemented by assigning new values to the variables \texttt{istate[}$p$\texttt{][}$i$\texttt{]}. These assignments may as before depend on the life stage that is entered, as specified by the variable \texttt{lifestage[]}, the (old values) of the individual state variables, contained in the argument \texttt{istate[][]}, the individual's state-at-birth, determined by the arguments \texttt{birthstate[][]} and \texttt{BirthStateNr}, and on the environment variables \texttt{E[]}. If no assignment of a value to \texttt{istate[}$p$\texttt{][}$i$\texttt{]} is implemented, that particular individual state variable will keep its current value.
...
>  output <- PSPMequi(modelname = NULL, biftype = NULL, startpoint = NULL, stepsize = NULL, 
                      ...
                      force  = FALSE, debug  = FALSE, silent = FALSE)
```
...
5. The fifth argument to the \texttt{PSPMequi} function determines which of the model parameters should be varied during the computation and at which parameter values the computations should stop. This information should be specified by a (row) vector, which for every parameter to be varied specifies a triplet of values including the index of the parameter, its minimum and its maximum value at which the computation should stop. For equilibrium computations (type \texttt{"EQ"}) the vector should hence have the following format:
    ...
    The first element of the vector indicates the index of the parameter in the vector \texttt{DefaultParameters} (in \texttt{R}, see section [4.3.1.4](#default-parameters-1)) or in the array \texttt{parameter} (in \texttt{C}, see section [4.3.2.2](#definition-of-parameter-names-and-values-1)) to vary, while the final two elements of the array indicate the minimum and maximum value of the parameter. The computation of the equilibrium curve as a function of the model parameter stops, whenever the minimum or maximum parameter value is reached.
	  ...
	  For all two-parameter type of computations (\texttt{"BP"}, \texttt{"BPE"}, \texttt{"LP"} and \texttt{"PIP"}) the vector should be extended with the index of the second parameter to vary in the computation, as well as the minimum and the maximum value of this parameter, at which to stop the curve computation. The vector with parameter information has therefore in this case the format:

	  \texttt{c(}$<$*index 1*$>$,$<$*minimum 1*$>$,$<$*maximum 1*$>$,$<$*index 2*$>$,$<$*minimum 2*$>$,$<$*maximum 2*$>$\texttt{)}

	  For \texttt{"ESS"} computations the vector of length 3 with the index, minimum and maximum value of the bifurcation parameter has to be extended with sets of 4 values for each of the parameters that are assumed to adopt their ESS value. Each set of 4 values indicates the index of the population that a particular ESS parameter influences, the index of the ESS parameter in the vector \texttt{DefaultParameters} (in \texttt{R}, see section [4.3.1.4](#default-parameters-1)) or in the array \texttt{parameter} (in \texttt{C}, see section [4.3.2.2](#definition-of-parameter-names-and-values-1)) and the minimum and maximum value of the parameter. In case only one parameter is to adopt its ESS value, the argument therefore has the form:
  
    \texttt{c(}$<$*index 1*$>$,$<$*minimum 1*$>$,$<$*maximum 1*$>$,$<$*population 2*$>$,$<$*index 2*$>$,$<$*minimum 2*$>$,$<$*maximum 2*$>$\texttt{)}
  
    If multiple parameters have their ESS value this vector has to include a set of 4 values for each of these model parameters. The number of sets should correspond with the number of initial estimates for the ESS values of parameters as specified in the third argument to the function. 
...
7. The seventh, optional argument of the \texttt{PSPMequi} function is a (row) vector of string elements, containing possible options that modify the behavior of the computational module. Some of the options require a value and hence occur as a pair of option name and option value, while others occur on their own. Options can be specified in any order, but the option value should always immediately follow after the option name. All option values refer to indices of either environment variables, structured populations or individual state variables. Notice, that this index value follows the \texttt{C}-convention of ordering arrays starting at 0 (as opposed to \texttt{R} where array indices start at 1). Multiple options can be included into the vector like:
    ...
    i) Option pair \texttt{c("report", "i")}: This option determines how much output the computational module reports to the console. Passing \texttt{c("report", "1")} as option vector to the \texttt{PSPMequi} function is the same as the default behaviour: the software writes the values of every new solution point that it has computed to the \texttt{R} console. Passing \texttt{c("report", "2")} as option vector to the \texttt{PSPMequi} function would make the program write every other computed solution point to the \texttt{R} console, while specifying \texttt{c("report", "10")} as option vector to the \texttt{PSPMequi} function implies that every 10th solution point that is computed is written to the \texttt{R} console.
...
* Option \texttt{silent}: When \texttt{silent=TRUE} is passed as argument, it instructs the \texttt{PSPMequi} function to suppress all messages from the compilation of the model specific file into a dynamic library module. This option is useful to prevent cluttering the console with superfluous messages once a model specific file has been tested sufficiently and functions without problems.
...
> The \texttt{PSPManalysis} package implements both the simplified EBT method proposed by Brännström, Carlsson and Simpson [-@Brannstrometal2013] as well as the original EBT method [@DeRoos1988, @DeRoosetal1992]. The simplified EBT method is faster, but possibly less accurate than the original EBT method. Switching between the two methods can be done by defining the \verb|EBTMETHOD| equal to 0 or 1 in the model implementation file. See also section [9.3](#optional-numerical-settings-2).
...
1. The sixth, optional argument of the \texttt{PSPMecodyn} function is a (row) vector of string elements, containing possible options that modify the behavior of the computational module. Only two options are possible and both require a value and hence occur as a pair of option name and option value. The option argument is therefore either of the form:
    ...
    i. Option pair \texttt{c("report", "}*i*\texttt{")}: This option, which should have a positive value *i*, determines the time interval between consecutive lines of data output to the console. The default behavior of the function \texttt{PSPMecodyn} is to write a line of output to the console whenever a new cohort of individuals is constructed. However, this may result in a lot of output. By choosing a higher value for this option the program can be forced to write to the console only every 10th time that a cohort is constructed. Notice that this option does not affect the frequency with which output is written to the output file with extension \texttt{.out} (see section [5.3](#output-files-generated-by-the-function-2) below).

Four other optional arguments can be passed to the \texttt{PSPMecodyn} function: \texttt{clean}, \texttt{force}, \texttt{debug} and \texttt{silent}. These are all boolean arguments that hence have to be passed to the \texttt{PSPMecodyn} function as $<$*option name*$>$\texttt{=TRUE} or $<$*option name*$>$\texttt{=FALSE}, the latter being the default value of all options (Specifying these options as argument is hence only useful when setting them equal to \texttt{TRUE}). Unlike the previous arguments, which all modify the computations to be performed, these options modify the behavior of the \texttt{PSPMecodyn} function itself, in particular the compilation of the model specific file into a dynamic library module that can be executed from \texttt{R}. Also unlike all the previous arguments that can be passed, these arguments can be passed in any order and at any position, the \texttt{PSPMecodyn} function will filter these 3 optional arguments from the argument list before passing the filtered argument list to the computational routine.
...
* Option \texttt{silent}: When \texttt{silent=TRUE} is passed as argument, it instructs the \texttt{PSPMecodyn} function to suppress all messages from the compilation of the model specific file into a dynamic library module. This option is useful to prevent cluttering the console with superfluous messages once a model specific file has been tested sufficiently and functions without problems.
...
## Additional remarks
The \texttt{PSPMecodyn} function allows for studying the dynamics of models can that can be analyzed with the other functions of the \texttt{PSPManalysis} package as well. In these models reproduction should be continuous in time. The package does allow to carry out demographic analysis for models in which reproduction is pulsed in time (see section [9.2](#periodic-environments)), but pulsed reproduction in combination with feedback of the environment on individual life history can not be dealt with by \texttt{PSPManalysis}. Moreover, the computations of the equilibrium state of a model, as carried out by the \texttt{PSPMequi} function, rely heavily on the assumption that the environmental variables are constant, which would exclude that the environmental variables exhibit seasonality or other time-dependent dynamics. However, the \texttt{PSPMecodyn} function can be used to study time-dependent dynamics. To allow for this a global variable called `Time` is defined that holds the current time value. This global variable can for example be used in the function specifying the dynamics of the environmental variables (`EnvEqui()`; see section [4.3.2.12](#specification-of-equilibrium-conditions-of-the-environment)). Notice that this global variable `Time` is always equal to 0 when using all other functions in the \texttt{PSPManalysis} package.
...
1. The seventh, optional argument of the \texttt{PSPMevodyn} function is a (row) vector of string elements, containing possible options that modify the behavior of the computational module. Most of the options require a value and hence occur as a pair of option name and option value. Only the \texttt{"test"} option (see below) occurs on its own. Options can be specified in any order, but the option value should always immediately follow after the option name. All option values refer to indices of either environment variables, structured populations or individual state variables. Notice, that this index value follows the \texttt{C}-convention of ordering arrays starting at 0 (as opposed to \texttt{R} where array indices start at 1). Multiple options can be included into the vector like:
    ...
    i. Option pair \texttt{c("report", "i")}: This option determines how much output the computational module reports to the console. Passing \texttt{c("report", "1")} as option vector to the \texttt{PSPMevodyn} function is the same as the default behaviour: the software writes the values of every new solution point that it has computed to the \texttt{R} console. Passing \texttt{c("report", "2")} as option vector to the \texttt{PSPMevodyn} function would make the program write every other computed solution point to the \texttt{R} console, while specifying \texttt{c("report", "10")} as option vector to the \texttt{PSPMevodyn} function implies that every 10th solution point that is computed is written to the \texttt{R} console.    
...
* Option \texttt{silent}: When \texttt{silent=TRUE} is passed as argument, it instructs the \texttt{PSPMevodyn} function to suppress all messages from the compilation of the model specific file into a dynamic library module. This option is useful to prevent cluttering the console with superfluous messages once a model specific file has been tested sufficiently and functions without problems.
...
* Option \texttt{silent}: When \texttt{silent=TRUE} is passed as argument, it instructs the \texttt{PSPMind} function to suppress all messages from the compilation of the model specific file into a dynamic library module. This option is useful to prevent cluttering the console with superfluous messages once a model specific file has been tested sufficiently and functions without problems.
...
\verb|EBTMETHOD|           1               Type of EBT method to use for simulating ecological
                                           dynamics. Possible values are 0 for using the
                                           simplified EBT method proposed by Brännström, 
                                           Carlsson and Simpson (2013) or 1 to use the original
                                           EBT method (de Roos (1988)).
