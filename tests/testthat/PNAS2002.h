/*
  PNAS2002.h -  Header file specifying the elementary life-history functions of
...
// The following lines of code are only executed in case the source file is used for
// demographic analysis (calculation of population growth rate and its sensitivities).
// Only in that case the macro PSPMDEMO is defined and equal to 1. In case of demographic
// analysis the environmental variables are not used and hence have to be given values.
// The statements below imply that the population growth rate of the consumer is defined
// in the absence of predators and with the resource equal to RMAX, the parameter
// representing the maximum resource density in the absence of consumers.
#if (PSPMDEMO == 1)
  R = RMAX;
  P = 0;
  B = 0;
...
{
  fecundity[0][0]                        = 0.0;
  if (lifestage[0] == 2) fecundity[0][0] = RM*R/(R + RH)*LENGTH*LENGTH;
